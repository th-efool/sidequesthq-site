    ...
    return (
        <section className={styles.hero} style={{ backgroundImage: `linear-gradient(90deg, rgb(8 20 16 / 94%) 0%, rgb(20 69 42 / 90%) 52%, rgb(62 158 89 / 82%) 100%), url(${cohort.coverImage})` }}>
            <div className={styles.artCard}>
                ...
                <Image src={cohort.coverImage} alt="" width={210} height={150} className={styles.artImage} />
            </div>
            ...
            <div className={styles.content}>
                ...
                <div className={styles.leaderRow}>
                    <Image src={cohort.creator.avatarUrl} alt="" width={42} height={42} />
                    <div>
            ...
            <aside className={styles.progressPanel}>
                <div className={styles.statsRow}>
                    <span><Users size={16} /> {cohort.stats.explorerCount.toLocaleString()} learners</span>
                    <span><Star size={16} /> {cohort.stats.rating.toFixed(1)}</span>
                </div>
                ...
                <div>
                    ...
                    <div className={styles.track}>
                        <div className={styles.bar} style={{ width: `${cohort.progress.journeyProgress}%` }} />
                    </div>
                </div>

                <a className={styles.resumeButton} href={`/play?cohort=${cohort.id}${currentQuest ? `&lesson=${currentQuest.id}` : ""}`}>
                    <Play size={16} fill="currentColor" />
                    Resume lesson
                </a>

                <div className={styles.actions}>
                    <a href={`/message?community=${cohort.id}`}><MessageCircle size={18} /> Discuss</a>
                    <button type="button" aria-label="Bookmark cohort"><Bookmark size={18} /></button>
                    <button type="button" aria-label="Share cohort"><Share2 size={18} /></button>
                </div>
            </aside>
        </section>
    );
