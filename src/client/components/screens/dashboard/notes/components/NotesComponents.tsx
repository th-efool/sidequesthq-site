...
export function CanvasSwitcher({
    ...
    const selectedIndex = Math.max(0, notes.findIndex((note) => note.id === selectedId));
    ...
    return (
        <div className={styles.canvasSwitcher}>
            ...
            {open && (
                <div className={styles.canvasMenu} role="listbox" onClick={(event) => event.stopPropagation()}>
...
export function Empty({ label, action, onClick }: { label: string; action: string; onClick: () => void }) {
    return (
...
function RenameInput({ value, onSave, onCancel }: { value: string; onSave: (value: string) => void; onCancel: () => void }) {
...
function BookRow({
    ...
    return (
        ...
        >
            ...
            >
                <Star size={14} fill={book.favorite ? "#fbbf24" : "none"} />
            </button>
            {menu === book.id && <Context book={book} actions={actions} />}
        </div>
    );
}
...
export function Notebook({
    ...
    return (
        <div className={styles.current}>
            ...
            />
            {!book.collapsed && book.visibleNotes.map((note) => (
                <button
                    key={note.id}
                    draggable
                ...
                >
                    <span />
                    <b className={styles.noteTitle} onDoubleClick={(event) => {
                        event.stopPropagation();
                    ...
                    {menu === note.id && (
                        <Menu>
                            <button onClick={() => actions.patchNote(note.id, { title: prompt("Rename note", note.title) || note.title })}>Rename</button>
                            <button onClick={() => actions.duplicateNote(note.id)}>Duplicate</button>
                            <button onClick={() => actions.patchNote(note.id, { favorite: !note.favorite })}>Favorite</button>
                            <button onClick={() => actions.archiveNote(note.id)}>{note.archived ? "Restore" : "Archive"}</button>
                            <button onClick={() => actions.deleteNote(note.id)}>Delete</button>
                        </Menu>
                    )}
                </button>
            ))}
            <button className={styles.addCanvas} onClick={() => actions.createNote(book.id)}>+ New canvas in this notebook</button>
        </div>
    );
}

function Context({ book, actions }: { book: NotebookListItem; actions: ReturnType<typeof useNotes>["actions"] }) {
    return (
        <Menu>
            <button onClick={() => actions.patchNotebook(book.id, { title: prompt("Rename notebook", book.title) || book.title })}>Rename</button>
            <button onClick={() => actions.duplicateNotebook(book.id)}>Duplicate</button>
            <button onClick={() => actions.patchNotebook(book.id, { collapsed: !book.collapsed })}>{book.collapsed ? "Expand" : "Collapse"}</button>
            <button onClick={() => actions.patchNotebook(book.id, { favorite: !book.favorite })}>Favorite</button>
            <button onClick={() => actions.archiveNotebook(book.id)}>{book.archived ? "Restore" : "Archive"}</button>
...
export function Menu({ children }: { children: ReactNode }) {
    return <div className={styles.menu} onClick={(event) => event.stopPropagation()}>{children}</div>;
}
        ...
        <div className={styles.scrim}>
            <div className={styles.modal}>
                ...
                <div className={styles.shareLine}>
                    ...
                    <select value={note.permission} onChange={(event) => onSave({ permission: event.target.value as Permission })}>
                ...
                <button onClick={() => navigator.clipboard?.writeText(`${location.origin}/notes/${note.id}`)}>Copy Link</button>
