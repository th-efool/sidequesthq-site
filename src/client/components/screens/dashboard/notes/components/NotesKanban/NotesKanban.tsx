...
export function NotesKanban({ noteId }: { noteId: string }) {
  ...
  const addColumn = () => {
    ...
    setColumns(prev => [...prev, { id, label: 'New Column' }]);
  ...
  const commitColRename = () => {
    ...
    if (trimmed) {
      setColumns(prev => prev.map(c => c.id === editingColId ? { ...c, label: trimmed } : c));
    }
