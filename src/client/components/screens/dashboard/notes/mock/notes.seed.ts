...
const titles: Record<string, string[]> = {
  ...
  "nb-reading": ["The Beginning of Infinity", "Designing Data-Intensive Applications", "Atomic Habits"],
};

export const seedNotebooks: NotebookEntity[] = notebookNames.map(([id, title, description, color], order) => ({
  id, title, description, color, order, collapsed: id !== "nb-ml", favorite: order % 4 === 0,
  shared: order === 1 || order === 8, archived: false, createdAt: daysAgo(40 - order), updatedAt: daysAgo(order + 1),
}));

export const seedNotes: NoteEntity[] = Object.entries(titles).flatMap(([notebookId, list], nbIndex) => list.map((title, order) => ({
