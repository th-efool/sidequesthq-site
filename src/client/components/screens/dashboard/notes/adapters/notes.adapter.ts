import type { NoteEntity, NotebookEntity, NotebookListItem, NotesFilter, NotesSort, NotesStateEntity } from "../models/notes.models";
...
export function sortItems<T extends { title: string; order: number; createdAt: string; updatedAt: string }>(items: T[], sort: NotesSort): T[] {
  ...
  if (sort === "recentlyCreated" || sort === "newestFirst") return copy.sort(byDate("createdAt", -1));
  if (sort === "oldestFirst") return copy.sort(byDate("createdAt", 1));
  return copy.sort((a, b) => a.order - b.order);
}

const isRecent = (item: { updatedAt: string }) => Date.now() - new Date(item.updatedAt).getTime() < 14 * 86400000;
const notebookFor = (state: NotesStateEntity, note: NoteEntity) => state.notebooks.find((book) => book.id === note.notebookId);
...
export function matchesFilter(state: NotesStateEntity, item: NoteEntity | NotebookEntity): boolean {
  ...
  if (state.filter === "favorites") return item.favorite || ("notebookId" in item && Boolean(notebookFor(state, item)?.favorite));
...
export function toNotebookItems(state: NotesStateEntity, notebookQuery: string, noteQuery = ""): NotebookListItem[] {
  ...
  return sortItems(state.notebooks, state.notebookSort)
    .filter((book) => matchesFilter(state, book) || notes.some((note) => note.notebookId === book.id))
