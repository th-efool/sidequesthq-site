...
export function calculatePriority<T extends { id: string }>(items: T[]): Array<T & { rank: number }> {
...
export function reorderCohorts(items: ActiveCohort[], draggedId: string, targetId: string): ActiveCohort[] {
...
export function updateSchedule(items: ActiveCohort[], cohortId: string, days: Weekday[]): ActiveCohort[] {
    const schedule = formatSchedule(days);

    return items.map((item) => item.id === cohortId ? { ...item, schedule } : item);
}

export function updateDailyGoal(items: ActiveCohort[], cohortId: string, dailyGoalMinutes: number): ActiveCohort[] {
    const safeGoal = Math.min(180, Math.max(5, dailyGoalMinutes));

    return items.map((item) => item.id === cohortId ? { ...item, dailyGoalMinutes: safeGoal } : item);
}
...
export function getActiveCohorts(activeItems: ActiveCohort[], pausedItems: PausedCohort[], today = new Date()) {
