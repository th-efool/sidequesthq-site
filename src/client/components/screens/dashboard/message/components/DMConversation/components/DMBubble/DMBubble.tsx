...
export function DMBubble({ message, user }: Props) {const outgoing = message.type === "outgoing"; return <article className={`${styles.row} ${outgoing ? styles.outgoing : styles.incoming}`}>{!outgoing && <span className={styles.avatar}>{message.showAvatar && <img src={user.avatar} alt=""/>}</span>}<div className={styles.wrap}><div className={`${styles.bubble} ${message.tail ? styles.tail : ""}`}>{message.replyTo && <em className={styles.reply}>Replying to {message.replyTo}</em>}{message.text && message.text.split("\n").map((line) => <span key={line}>{line}</span>)}{message.attachment && <MessageAttachment attachment={message.attachment}/>}</div><div className={styles.meta}>{message.timestamp}<MessageStatus status={message.status}/></div><MessageReaction reactions={message.reactions} outgoing={outgoing}/></div></article>}
