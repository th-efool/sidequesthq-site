...
export function CommunityChat({ community, draft, scrollTop, onBack, onDraftChange, onScrollChange, onSend, onUpload }: Props) {
    ...
    useEffect(() => {
        if (!aboutOpen) return;
        const onKey = (event: KeyboardEvent) => { if (event.key === "Escape") setAboutOpen(false); };
        const onPointer = (event: MouseEvent) => { const target = event.target as HTMLElement; if (!target.closest("[data-about-panel]") && !target.closest("[data-about-toggle]")) setAboutOpen(false); };
