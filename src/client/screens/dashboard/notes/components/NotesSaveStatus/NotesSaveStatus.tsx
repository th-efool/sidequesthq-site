...
interface NotesSaveStatusProps {
  state?: CanvasState;
  providerStatus?: 'connected' | 'connecting' | 'disconnected' | 'syncing' | null;
}
...
export function NotesSaveStatus({ state, providerStatus }: NotesSaveStatusProps) {
  if (providerStatus !== undefined) {
    ...
    if (providerStatus === 'disconnected') {
      return (
        <div className={styles.container}>
          <span className={styles.error} title="Disconnected from collaboration server">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
